...
func newPaperRuntime(cfg runtimeConfig,app *application,store runtimeStore,provider runtimeRequestProvider)(*paperRuntime,error){if e:=cfg.validate();e!=nil{return nil,e};b:=make([]byte,16);if _,e:=rand.Read(b);e!=nil{return nil,e};r:=&paperRuntime{cfg:cfg,app:app,store:store,provider:provider,owner:hex.EncodeToString(b),now:time.Now,queue:make(chan runtimeCycle,cfg.QueueCapacity)};r.status=runtimeStatus{Enabled:cfg.Enabled,State:"disabled",OwnerID:r.owner,ConfigFingerprint:cfg.fingerprint(),LastSuccess:map[string]time.Time{},Warning:runtimeWarning};return r,nil}
